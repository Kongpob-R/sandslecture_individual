    ...
    if request.GET.get('word'):
        ...
        for note in Lecture.objects.all():
            if keyword in note.title.lower() or keyword in note.description.lower():
                matchNote.append(NoteWithThumbnail(note, note.Lecture_img.all()[0]))
    ...
    if Profile.objects.filter(user=request.user):
        ...
        if request.method == 'POST':
            ...
            if LectureForm.is_valid():
                ...
                LectureForm.author = profileObject
                LectureForm.save()

                for imageFile in request.FILES.getlist('image'):
                    photo = Lecture_img.objects.create(LectureKey=LectureForm, image=imageFile)
                    photo.save()
...
def noteView(request, lecture_id):
    if request.method == 'POST':
        profileObject = Profile.objects.get(user = request.user)
        noteObject = Lecture.objects.get(id = int(request.POST.get('noteID')))
        ...
        noteObject = Lecture.objects.get(id = lecture_id)
        imageObjectList = noteObject.Lecture_img.all()
